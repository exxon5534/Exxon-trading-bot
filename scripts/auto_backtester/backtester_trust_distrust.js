...
/* exxon 4.04 Backtester v0.2
 * Ali Anari <ali@anari.io>
 * 05/30/2017
 *
 * Usage: Pass in the same parameters as you would to "exxon sim", EXCEPT for:
...
let VERSION = 'exxon 4.04 Backtester v0.2'
...
let runCommand = (strategy, cb) => {
  countArr.push(1)
  let command = `./exxon.sh sim ${simArgs} --strategy=trust_distrust --period_length=${strategy.period_length}m --sell_threshold=${strategy.sell_threshold} --sell_threshold_max=${strategy.sell_threshold_max} --sell_min=${strategy.sell_min} --buy_threshold=${strategy.buy_threshold} --days=30`
