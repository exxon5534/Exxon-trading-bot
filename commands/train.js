  ...
  program
    ...
    .action(function (selector, cmd) {        
      ...
      function trainingDone (strategy, lastPeriod) {
        ...
        var exxon_cmd = process.platform === 'win32' ? 'exxon.bat' : 'exxon.sh' // Use 'win32' for 64 bit windows too
        var trainingArgs = [
          'sim',
        ...
        var trainingSimulation = spawn(path.resolve(__dirname, '..', exxon_cmd), trainingArgs, { stdio: 'inherit' })
        ...
        trainingSimulation.on('exit', function (code, signal) {
          ...
          if (so.days_test > 0) {
            ...
            var testSimulation = spawn(path.resolve(__dirname, '..', exxon_cmd), testArgs, { stdio: 'inherit' })
      ...
      function createCandlesticks () {
        ...
        if (!s.period) {
          console.error('no trades found! try running `exxon backfill ' + so.selector.normalized + '` first')
          process.exit(1)
        }
